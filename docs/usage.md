# nf-core/splitspliceleaderpe: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/splitspliceleaderpe/usage](https://nf-co.re/splitspliceleaderpe/usage)
...
```console
nextflow run nf-core/splitspliceleaderpe --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/splitspliceleaderpe
```
...
First, go to the [nf-core/splitspliceleaderpe releases page](https://github.com/nf-core/splitspliceleaderpe/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
